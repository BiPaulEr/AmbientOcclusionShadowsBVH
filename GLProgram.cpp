...
void GLProgram::link () {
    ...
    if (!linked)
        throw Exception (name() + " Shaders not linked: " + infoLog ());
}
...
GLProgram * GLProgram::genVFProgram(const std::string & name,
	...
	const std::string & fragmentShaderFilename, const std::string* controlTessalationFilename, const std::string* tessalationShaderFilename) {
	...
	GLShader * ct = (controlTessalationFilename) ? new GLShader(name + "Tesselllation Control", GL_TESS_CONTROL_SHADER) : nullptr;
	GLShader * ts = (tessalationShaderFilename) ? new GLShader(name + "Tessellation Shader", GL_TESS_EVALUATION_SHADER) : nullptr;
    vs->loadFromFile (vertexShaderFilename);
    vs->compile ();
    p->attach(vs);
	if (ct)
	{
		ct->loadFromFile(*controlTessalationFilename);
		ct->compile();
		p->attach(ct);
	}
	if (ts)
	{
		ts->loadFromFile(*tessalationShaderFilename);
		ts->compile();
		p->attach(ts);
	}
